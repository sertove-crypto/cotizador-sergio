    ...
    <div className="min-h-screen pb-44 bg-[#F8FAFB] font-sans animate-fade-in overflow-x-hidden">
        ...
        <div className="max-w-md mx-auto flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">v1.4 - Actualización Final</p>
            </div>
          </div>
      ...
      <main className="max-w-md mx-auto px-5 pt-10 space-y-12">
        ...
        <section className="space-y-6">
            ...
            <div className="space-y-8">
              <div className="bg-white rounded-[3rem] p-6 shadow-xl border border-slate-50">
                ...
                <div className="space-y-4">
                  {seats.map((s, i) => (
                    <div key={i} className={`grid grid-cols-[1fr_1fr_1fr_auto] items-center gap-2 p-3 rounded-[1.8rem] transition-all ${s.qty > 0 ? 'bg-teal-50 ring-2 ring-teal-500/20' : 'bg-slate-50'}`}>
                      <input type="number" value={s.w} onChange={(e)=>{const n=[...seats]; n[i].w=Number(e.target.value); setSeats(n)}} className="w-full p-3 bg-white rounded-xl text-center text-sm font-bold border border-slate-200" placeholder="L" />
                      <input type="number" value={s.h} onChange={(e)=>{const n=[...seats]; n[i].h=Number(e.target.value); setSeats(n)}} className="w-full p-3 bg-white rounded-xl text-center text-sm font-bold border border-slate-200" placeholder="An" />
                      <input type="number" value={s.t} onChange={(e)=>{const n=[...seats]; n[i].t=Number(e.target.value); setSeats(n)}} className="w-full p-3 bg-white rounded-xl text-center text-sm font-bold border border-slate-200" placeholder="Es" />
              ...
              <div className="bg-white rounded-[3rem] p-6 shadow-xl border border-slate-50">
                ...
                <div className="space-y-4">
                  {backrests.map((b, i) => (
                    <div key={i} className={`grid grid-cols-[1fr_1fr_1fr_auto] items-center gap-2 p-3 rounded-[1.8rem] transition-all ${b.qty > 0 ? 'bg-teal-50 ring-2 ring-teal-500/20' : 'bg-slate-50'}`}>
                      <input type="number" value={b.w} onChange={(e)=>{const n=[...backrests]; n[i].w=Number(e.target.value); setBackrests(n)}} className="w-full p-3 bg-white rounded-xl text-center text-sm font-bold border border-slate-200" placeholder="L" />
                      <input type="number" value={b.h} onChange={(e)=>{const n=[...backrests]; n[i].h=Number(e.target.value); setBackrests(n)}} className="w-full p-3 bg-white rounded-xl text-center text-sm font-bold border border-slate-200" placeholder="An" />
                      <input type="number" value={b.t} onChange={(e)=>{const n=[...backrests]; n[i].t=Number(e.target.value); setBackrests(n)}} className="w-full p-3 bg-white rounded-xl text-center text-sm font-bold border border-slate-200" placeholder="Es" />
          ...
          {activeTab === 'colchoneta' && (
            <div className="space-y-8">
              ...
              <div className="bg-white rounded-[3rem] p-6 shadow-xl border border-slate-50">
                <span className="text-[11px] font-black text-[#005F6B] uppercase block mb-1 ml-3 tracking-[0.2em]">Especiales (Mayor 4 pzas)</span>
                <div className="space-y-4">
                  {customMattresses.map((m, i) => (
                    <div key={i} className={`grid grid-cols-[1fr_1fr_1fr_auto] items-center gap-2 p-3 rounded-[1.8rem] transition-all ${m.qty >= 4 ? 'bg-teal-50 ring-2 ring-teal-500/20' : 'bg-slate-50'}`}>
                      <input type="number" value={m.w || ''} onChange={(e)=>{const n=[...customMattresses]; n[i].w=Number(e.target.value); setCustomMattresses(n)}} className="w-full p-3 bg-white rounded-xl text-center text-sm font-bold border border-slate-200" placeholder="L" />
                      <input type="number" value={m.h || ''} onChange={(e)=>{const n=[...customMattresses]; n[i].h=Number(e.target.value); setCustomMattresses(n)}} className="w-full p-3 bg-white rounded-xl text-center text-sm font-bold border border-slate-200" placeholder="An" />
                      <input type="number" value={m.t || ''} onChange={(e)=>{const n=[...customMattresses]; n[i].t=Number(e.target.value); setCustomMattresses(n)}} className="w-full p-3 bg-white rounded-xl text-center text-sm font-bold border border-slate-200" placeholder="Es" />
          ...
          <div className="space-y-10">
            ...
            {activeTab === 'colchoneta' ? (
              <div className="bg-white p-5 rounded-2xl shadow-md border border-slate-200 flex items-center gap-3">
                ...
                <span className="text-[#005F6B] font-black text-xs uppercase tracking-widest">Esponja Premium + Sintético Impermeable</span>
              </div>
            ) : (
              <div className="flex flex-col gap-4">
                <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest block ml-2">Gama de Tela</span>
                <div className="flex bg-slate-100 p-1.5 rounded-2xl">
                  {(['A', 'B'] as FabricGroup[]).map(g => (
                    <button key={g} onClick={() => activeTab === 'cojin' ? setCushionFabricGroup(g) : setMuebleFabricGroup(g)} className={`flex-1 py-3 rounded-xl text-[10px] font-black transition-all ${(activeTab === 'cojin' ? cushionFabricGroup : muebleFabricGroup) === g ? 'bg-white text-[#005F6B] shadow-md' : 'text-slate-400'}`}>
        ...
        <section className="relative px-2">
          <div className="bg-[#020617] rounded-[4.5rem] p-12 shadow-2xl text-white overflow-hidden relative border border-white/5">
            <div className="relative z-10 space-y-12">
              <div className="space-y-2">
                <span className="text-[11px] font-black text-teal-400 uppercase tracking-[0.4em] opacity-80">Resumen v1.4</span>
                    ...
                    <div key={idx} className="flex justify-between items-center">
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="text-[10px] text-white/30 uppercase font-black tracking-widest">
                          {item.qty} pzas • {item.isMattress ? 'Calidad Premium' : `Gama ${item.group === FabricGroup.B ? 'Premium' : 'Estándar'}`}
                        </span>
                      </div>
        ...
        <section className="bg-white rounded-[3rem] p-10 shadow-xl border border-slate-50 space-y-8">
          ...
          <div className="space-y-5">
            <input type="text" placeholder="Tu nombre" value={customer.name} onChange={e=>setCustomer({...customer, name:e.target.value})} className="w-full p-7 bg-slate-50 rounded-[2.2rem] font-black text-lg outline-none border-2 border-transparent focus:border-[#005F6B] transition-all placeholder:text-slate-300" />
            <input type="tel" placeholder="Tu celular" value={customer.phone} onChange={e=>setCustomer({...customer, phone:e.target.value})} className="w-full p-7 bg-slate-50 rounded-[2.2rem] font-black text-lg outline-none border-2 border-transparent focus:border-[#005F6B] transition-all placeholder:text-slate-300" />
          </div>
        </section>
      </main>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-8 bg-white/95 backdrop-blur-3xl border-t border-slate-100 z-[60] flex justify-center shadow-2xl">
        <button 
          ...
          className="w-full max-w-md py-8 rounded-[3rem] bg-[#25D366] text-white font-black text-lg uppercase tracking-[0.35em] shadow-2xl active:scale-95 disabled:grayscale disabled:opacity-20 transition-all flex items-center justify-center gap-4 animate-pulse-subtle"
        >
      ...
      <style>{`
        @keyframes pulse-subtle {
          ...
          50% { transform: scale(1.02); box-shadow: 0 0 40px rgba(37, 211, 102, 0.4); }
        }
